...
/**
 * Route & Page-Level Error Boundary for Virtual Labs.
 * Prevents any unhandled runtime render error from crashing the application into a white screen.
 */
export default class VirtualLabsErrorBoundary extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      hasError: false,
  ...
  componentDidCatch(error, errorInfo) {
    // Log the actual error and stack trace for development/telemetry
    console.error('VirtualLabsErrorBoundary caught an error:', error, errorInfo);
  ...
  render() {
    if (this.state.hasError) {
      ...
      return (
        <div className="w-full min-h-[420px] flex items-center justify-center p-6 bg-gov-off-white">
          <div className="gov-card max-w-lg w-full p-8 text-center bg-white border border-gov-gray-200 shadow-gov-card rounded-gov-lg space-y-5">
            ...
            <div className="space-y-2">
              <h2 className="text-xl font-black text-gov-navy tracking-tight">
                Virtual Labs couldn't be loaded
              </h2>
              <p className="text-sm text-gov-gray-600 leading-relaxed max-w-md mx-auto">
                Something unexpected happened while opening this laboratory.
              </p>
            </div>
